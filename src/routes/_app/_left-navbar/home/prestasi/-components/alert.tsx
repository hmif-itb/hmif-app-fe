...
export function Alert({
  ...
  return (
    <div
      className={`fixed right-0 top-4 z-50 mx-4 max-w-md rounded-lg border-l-8 bg-white p-4 shadow-xl transition-all duration-300 ease-in-out${isAnimating ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-gray-400">
            {type === 'success'
              ? 'Submisi prestasi anda telah berhasil ditambahkan'
              : 'Submisi prestasi anda gagal untuk ditambahkan'}
          </p>
        </div>
        <button
          onClick={handleClose}
