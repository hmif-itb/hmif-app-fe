...
function HouseholdAdminPage() {
  ...
  const queryFilters = useMemo(() => {
    const conditionMap: Record<string, 'good' | 'broken' | 'cant_be_used' | 'lost'> = {
      good: 'good',
      hilang: 'lost',
      broken: 'broken',
    ...
    return {
      ...
      condition:
        filter.condition === 'all'
          ? undefined
          : (conditionMap[filter.condition] as 'good' | 'broken' | 'cant_be_used' | 'lost'),
    };
  }, [searchTerm, activeView, filter.condition]);
  

  const { data: propertiData = [], isLoading } = useGetPropertiList(queryFilters);
    ...
    <div className="flex h-full flex-col lg:px-10 lg:pb-[60px]">
      ...
      <main className="flex size-full flex-col gap-3 overflow-y-scroll bg-[#30764B] p-[34px] lg:items-center lg:gap-6 lg:rounded-xl lg:px-[26px]">
        <h1 className="flex items-center gap-3 text-[32px] font-bold text-white lg:text-center lg:text-5xl">
          <ChevronLeft className="size-9 lg:hidden" onClick={() => router.history.back()} />
          Manajemen Properti
        </h1>
        <SearchBar
          onFilterChange={setFilter}
          onSearchChange={setSearchTerm}
          onCreateProperty={() => {console.log("Opening property modal"); setCreatePropertyModalOpen(true);}}
        ...
        <SwitchToggle
          ...
          onValueChange={(value) => setActiveView(value.toLowerCase() as 'properti' | 'sekre')}
        />
